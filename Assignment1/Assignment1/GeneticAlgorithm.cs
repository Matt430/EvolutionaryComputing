...
namespace Assignment1
{
    class GeneticAlgorithm
    {
        ...
        public GeneticAlgorithm(int populationCount, int stringLength, FitnessFunction fitnessFunction, Crossover crossover)
        {
            ...
            List<bool> optimalSolution = new List<bool>();
            for(int i = 0; i < stringLength; ++i)
                optimalSolution.Add(true);
            optimalFitness = fitnessFunction.Fitness(optimalSolution);
        }
        ...
        public void Run()
        {
            ...
            while(PrintString(population[0]) != PrintString(population[populationCount - 1]))
            {
                ...
                //Writeline for debug.
                if(!firstHit && optimalFitness == fitnessFunction.Fitness(population[populationCount - 1]))
                {
                    firstHitGeneration = generation;
                    //Console.WriteLine("Found optimal solution at generation: " + generation);
                    firstHit = true;
                }
                //Console.WriteLine("Generation " + generation + ": " + PrintString(population[populationCount - 1]));
            }
